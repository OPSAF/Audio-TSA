...
def _test_arch_effects(returns: np.ndarray, nlags: int = 10) -> Tuple[float, bool]:
    """
    Ljung-Box test on squared returns to detect ARCH effects.

    If squared returns show no significant autocorrelation, there are
    no ARCH effects — GARCH fitting is pointless and will just fight noise.
    ...
    try:
        r2 = returns ** 2
        acf = np.correlate(r2 - r2.mean(), r2 - r2.mean(), mode="full")
        acf = acf[n - 1:] / acf[n - 1]  # normalise
        acf = acf[1:nlags + 1]           # skip lag 0
        # Ljung-Box Q statistic
        Q = float(n * (n + 2) * np.sum(acf ** 2 / (n - np.arange(1, nlags + 1))))
        from scipy.stats import chi2
        p_value = float(1 - chi2.cdf(Q, nlags))
        has_arch = p_value < 0.10  # standard 10% significance for ARCH pre-test
        return p_value, has_arch
    except Exception:
        return 0.5, True  # on error, assume ARCH effects exist (safe side)
...
def _fit_garch(series: np.ndarray, p: int = 1, q: int = 1) -> Dict:
    ...
    # Pre-check: if no ARCH effects exist, skip expensive GARCH fitting
    # and return constant volatility immediately.  This avoids the
    # "Model is not converging" noise from the optimizer fighting white noise.
    _pval, has_arch = _test_arch_effects(returns)
    if not has_arch:
        result = _make_constant_vol(returns)
        result["backend"] = "constant (no ARCH effects detected)"
        return result
